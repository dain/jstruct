...
public class UnsafeMemoryMappedFile implements Closeable {
    ...
    public UnsafeMemoryMappedFile(File file, boolean writable) throws IOException, OutOfMemoryError {
    ...
    public UnsafeMemoryMappedFile(FileChannel channel, MapMode mode) throws IOException, OutOfMemoryError {
        this(channel, mode, 0, channel.size());
    }

    public UnsafeMemoryMappedFile(FileChannel channel, MapMode mode, long position, long size) throws IOException, OutOfMemoryError {
        if (channel == null) {
    ...
    private long mmap0(int prot, long position, long length) throws IOException {
        try {
